import {
  ...
  watchConnectors,
  writeContract
} from "./chunk-UKN3GMJN.js";
import {
  import_index
} from "./chunk-7PFUWWMZ.js";
...
import {
  ...
  ProviderUtil,
  WcHelpersUtil
} from "./chunk-CWXS7TMC.js";
import "./chunk-3F7IWAT5.js";
...
import {
  ...
  withRetry,
  withTimeout
} from "./chunk-GJYFBT5A.js";
import {
  hmac
} from "./chunk-RI7MUXOU.js";
import "./chunk-YA3PZWKQ.js";
import "./chunk-TO2JMQY6.js";
import "./chunk-4TI4S7GE.js";
import {
  AlertController,
  ChainController,
  ConstantsUtil,
...
import {
  mainnet
} from "./chunk-R4KQFCSW.js";
import "./chunk-REQNGFPT.js";
import {
  HashMD,
  sha256
...
function createCurve(curveDef, defHash) {
  const create3 = (hash8) => weierstrass({ ...curveDef, ...getHash(hash8) });
...
function bls(CURVE) {
  const { Fp: Fp4, Fr: Fr2, Fp2: Fp23, Fp6: Fp62, Fp12: Fp122 } = CURVE.fields;
  ...
  const Fp2div2 = Fp23.div(Fp23.ONE, Fp23.mul(Fp23.ONE, _2n5));
  function pointDouble(ell, Rx, Ry, Rz) {
    const t0 = Fp23.sqr(Ry);
    const t1 = Fp23.sqr(Rz);
    const t2 = Fp23.mulByB(Fp23.mul(t1, _3n3));
    const t3 = Fp23.mul(t2, _3n3);
    const t4 = Fp23.sub(Fp23.sub(Fp23.sqr(Fp23.add(Ry, Rz)), t1), t0);
    const c0 = Fp23.sub(t2, t0);
    const c1 = Fp23.mul(Fp23.sqr(Rx), _3n3);
    const c2 = Fp23.neg(t4);
    ell.push([c0, c1, c2]);
    Rx = Fp23.mul(Fp23.mul(Fp23.mul(Fp23.sub(t0, t3), Rx), Ry), Fp2div2);
    Ry = Fp23.sub(Fp23.sqr(Fp23.mul(Fp23.add(t0, t3), Fp2div2)), Fp23.mul(Fp23.sqr(t2), _3n3));
    Rz = Fp23.mul(t0, t4);
    return { Rx, Ry, Rz };
  }
  function pointAdd(ell, Rx, Ry, Rz, Qx, Qy) {
    ...
    const t5 = Fp23.add(Fp23.sub(t3, Fp23.mul(t4, _2n5)), Fp23.mul(Fp23.sqr(t0), Rz));
    Rx = Fp23.mul(t1, t5);
...
function psiFrobenius(Fp4, Fp23, base) {
  const PSI_X = Fp23.pow(base, (Fp4.ORDER - _1n7) / _3n4);
  const PSI_Y = Fp23.pow(base, (Fp4.ORDER - _1n7) / _2n6);
  function psi(x, y) {
    const x2 = Fp23.mul(Fp23.frobeniusMap(x, 1), PSI_X);
    const y2 = Fp23.mul(Fp23.frobeniusMap(y, 1), PSI_Y);
    return [x2, y2];
  }
  const PSI2_X = Fp23.pow(base, (Fp4.ORDER ** _2n6 - _1n7) / _3n4);
  const PSI2_Y = Fp23.pow(base, (Fp4.ORDER ** _2n6 - _1n7) / _2n6);
...
function tower12(opts) {
  ...
  const Fp23 = {
    ...
    pow: (num2, power) => FpPow(Fp23, num2, power),
    invertBatch: (nums) => FpInvertBatch(Fp23, nums),
    // Normalized
    add: Fp2Add,
    sub: Fp2Subtract,
    mul: Fp2Multiply,
    ...
    div: (lhs, rhs) => Fp23.mul(lhs, typeof rhs === "bigint" ? Fp4.inv(Fp4.create(rhs)) : Fp23.inv(rhs)),
    inv: ({ c0: a, c1: b }) => {
    ...
    sqrt: (num2) => {
      ...
      if (Fp4.is0(c1)) {
        if (Fp4.eql(FpLegendre2(Fp4, c0), Fp4.ONE))
          ...
          return Fp23.create({ c0: Fp4.ZERO, c1: Fp4.sqrt(Fp4.div(c0, FpNONRESIDUE)) });
      }
      ...
      if (!Fp4.is0(legendre) && !Fp4.eql(legendre, Fp4.ONE))
        d = Fp4.sub(d, a);
      const a0 = Fp4.sqrt(d);
      const candidateSqrt = Fp23.create({ c0: a0, c1: Fp4.div(Fp4.mul(c1, Fpdiv2), a0) });
      if (!Fp23.eql(Fp23.sqr(candidateSqrt), num2))
        throw new Error("Cannot find square root");
    ...
    // multiply by u + 1
    mulByNonresidue: ({ c0, c1 }) => Fp23.mul({ c0, c1 }, Fp2Nonresidue),
  ...
  const Fp6Square = ({ c0, c1, c2 }) => {
    let t0 = Fp23.sqr(c0);
    let t1 = Fp23.mul(Fp23.mul(c0, c1), _2n6);
    let t3 = Fp23.mul(Fp23.mul(c1, c2), _2n6);
    let t4 = Fp23.sqr(c2);
    return {
  ...
  const [FP6_FROBENIUS_COEFFICIENTS_1, FP6_FROBENIUS_COEFFICIENTS_2] = calcFrobeniusCoefficients(Fp23, Fp2Nonresidue, Fp4.ORDER, 6, 2, 3);
  const Fp62 = {
    ORDER: Fp23.ORDER,
    // TODO: unused, but need to verify
    BITS: 3 * Fp23.BITS,
    BYTES: 3 * Fp23.BYTES,
    MASK: bitMask(3 * Fp23.BITS),
    ZERO: { c0: Fp23.ZERO, c1: Fp23.ZERO, c2: Fp23.ZERO },
    ONE: { c0: Fp23.ONE, c1: Fp23.ZERO, c2: Fp23.ZERO },
    create: (num2) => num2,
    isValid: ({ c0, c1, c2 }) => Fp23.isValid(c0) && Fp23.isValid(c1) && Fp23.isValid(c2),
    is0: ({ c0, c1, c2 }) => Fp23.is0(c0) && Fp23.is0(c1) && Fp23.is0(c2),
    neg: ({ c0, c1, c2 }) => ({ c0: Fp23.neg(c0), c1: Fp23.neg(c1), c2: Fp23.neg(c2) }),
    eql: ({ c0, c1, c2 }, { c0: r0, c1: r1, c2: r2 }) => Fp23.eql(c0, r0) && Fp23.eql(c1, r1) && Fp23.eql(c2, r2),
    sqrt: notImplemented,
    ...
    inv: ({ c0, c1, c2 }) => {
      ...
      return { c0: Fp23.mul(t4, t0), c1: Fp23.mul(t4, t1), c2: Fp23.mul(t4, t2) };
    },
    // Bytes utils
    fromBytes: (b) => {
      if (b.length !== Fp62.BYTES)
        throw new Error(`fromBytes wrong length=${b.length}`);
      return {
        c0: Fp23.fromBytes(b.subarray(0, Fp23.BYTES)),
        c1: Fp23.fromBytes(b.subarray(Fp23.BYTES, 2 * Fp23.BYTES)),
        c2: Fp23.fromBytes(b.subarray(2 * Fp23.BYTES))
      };
    },
    toBytes: ({ c0, c1, c2 }) => concatBytes2(Fp23.toBytes(c0), Fp23.toBytes(c1), Fp23.toBytes(c2)),
    ...
    frobeniusMap: ({ c0, c1, c2 }, power) => ({
      ...
      c1: Fp23.mul(Fp23.frobeniusMap(c1, power), FP6_FROBENIUS_COEFFICIENTS_1[power % 6]),
      c2: Fp23.mul(Fp23.frobeniusMap(c2, power), FP6_FROBENIUS_COEFFICIENTS_2[power % 6])
    }),
    ...
    mulByNonresidue: ({ c0, c1, c2 }) => ({ c0: Fp23.mulByNonresidue(c2), c1: c0, c2: c1 }),
  ...
  const FP12_FROBENIUS_COEFFICIENTS = calcFrobeniusCoefficients(Fp23, Fp2Nonresidue, Fp4.ORDER, 12, 1, 6)[0];
  ...
  function Fp4Square2(a, b) {
    ...
    const b2 = Fp23.sqr(b);
    return {
      first: Fp23.add(Fp23.mulByNonresidue(b2), a2),
      // b² * Nonresidue + a²
      second: Fp23.sub(Fp23.sub(Fp23.sqr(Fp23.add(a, b)), a2), b2)
      // (a + b)² - a² - b²
    };
  }
  const Fp122 = {
    ...
    mul034: ({ c0, c1 }, o0, o3, o4) => {
      ...
      const e = Fp62.mul01(Fp62.add(c0, c1), Fp23.add(o0, o3), o4);
      return {
  ...
  return { Fp: Fp4, Fp2: Fp23, Fp6: Fp62, Fp4Square: Fp4Square2, Fp12: Fp122 };
}
...
var { Fp: Fp2, Fp2: Fp22, Fp6, Fp4Square, Fp12 } = tower12({
  ...
  Fp2mulByB: ({ c0, c1 }) => {
    const t0 = Fp2.mul(c0, _4n3);
    const t1 = Fp2.mul(c1, _4n3);
...
var G2_SWU = mapToCurveSimpleSWU(Fp22, {
  A: Fp22.create({ c0: Fp2.create(_0n8), c1: Fp2.create(BigInt(240)) }),
  // A' = 240 * I
...
var COMPRESSED_ZERO = setMask(Fp2.toBytes(_0n8), { infinity: true, compressed: true });
...
function signatureG1ToRawBytes(point) {
  ...
  return setMask(numberToBytesBE(x, Fp2.BYTES), { compressed: true, sort });
}
function signatureG2ToRawBytes(point) {
  ...
  const sort = Boolean(tmp / Fp2.ORDER & _1n8);
  const z2 = x0;
...
var bls12_381 = bls({
  ...
  G1: {
    ...
    isTorsionFree: (c, point) => {
      ...
      const phi = new c(Fp2.mul(point.px, cubicRootOfUnityModP), point.py, point.pz);
    ...
    fromBytes: (bytes2) => {
      ...
      if (value.length === 48 && compressed) {
        ...
        const x = Fp2.create(compressedValue & Fp2.MASK);
        if (infinity) {
          if (x !== _0n8)
            throw new Error("G1: non-empty compressed point at infinity");
          return { x: _0n8, y: _0n8 };
        }
        const right = Fp2.add(Fp2.pow(x, _3n5), Fp2.create(bls12_381.params.G1b));
        ...
        const x = bytesToNumberBE(value.subarray(0, Fp2.BYTES));
        const y = bytesToNumberBE(value.subarray(Fp2.BYTES));
        if (infinity) {
          if (x !== _0n8 || y !== _0n8)
    ...
    toBytes: (c, point, isCompressed) => {
      ...
      if (isCompressed) {
        ...
        if (isZero) {
          const x2 = concatBytes2(new Uint8Array([64]), new Uint8Array(2 * Fp2.BYTES - 1));
          return x2;
        } else {
          return concatBytes2(numberToBytesBE(x, Fp2.BYTES), numberToBytesBE(y, Fp2.BYTES));
        }
      }
    },
    ShortSignature: {
  ...
  G2: {
    ...
    Signature: {
      ...
      fromHex(hex2) {
        ...
        const x1 = Fp2.create(z1 & Fp2.MASK);
...
var secp256r1 = p256;
var mapSWU2 = (() => mapToCurveSimpleSWU(Fp3, {
  A: CURVE_A,
  B: CURVE_B,
    ...
    async getProvider() {
      if (!walletProvider) {
        const CoinbaseWalletSDK = await (async () => {
          const SDK = await import("./dist-XTHUPDBW.js");
          if (typeof SDK.CoinbaseWalletSDK !== "function" && typeof SDK.default === "function")
            return SDK.default;
...
function version3(parameters) {
  ...
  return createConnector((config) => ({
    ...
    async getProvider() {
      ...
      if (!walletProvider) {
        const CoinbaseWalletSDK = await (async () => {
          const { default: SDK } = await import("./dist-CWLCJ2H7.js");
    ...
    async getProvider() {
      async function initProvider() {
        ...
        const MetaMaskSDK = await (async () => {
          const { default: SDK } = await import("./metamask-sdk-CRZSY752.js");
          if (typeof SDK !== "function" && typeof SDK.default === "function")
            return SDK.default;
          return SDK;
        })();
  ...
  return createConnector((config) => ({
    ...
    async getProvider() {
      ...
      if (!isIframe)
        return;
      if (!provider_) {
        const { default: SDK } = await import("./esm-ZDNHL4XW.js");
        ...
        const SafeAppProvider = await (async () => {
          const Provider = await import("./dist-C6UMNRJQ.js");
  ...
  return createConnector((config) => ({
    ...
    async getProvider({ chainId } = {}) {
      ...
      async function initProvider() {
        ...
        if (!optionalChains.length)
          return;
        const { EthereumProvider } = await import("./index.es-SHR3KVXB.js");
...
var WagmiAdapter = class {
  ...
  construct(appKit, options) {
    ...
    this.connectionControllerClient = {
      connectWalletConnect: async () => {
        ...
        const clientId = await ((_c2 = (_b2 = (_a2 = provider == null ? void 0 : provider.client) == null ? void 0 : _a2.core) == null ? void 0 : _b2.crypto) == null ? void 0 : _c2.getClientId());
        if (clientId) {
          (_d2 = this.appKit) == null ? void 0 : _d2.setClientId(clientId);
        }
        const siweParams = await ((_g = (_f = (_e = this.options) == null ? void 0 : _e.siweConfig) == null ? void 0 : _f.getMessageParams) == null ? void 0 : _g.call(_f));
        const isSiweEnabled = (_j = (_i = (_h = this.options) == null ? void 0 : _h.siweConfig) == null ? void 0 : _i.options) == null ? void 0 : _j.enabled;
        ...
        if (isSiweEnabled && isProviderSupported && isSiweParamsValid && siweConfig) {
          ...
          const { SIWEController, getDidChainId, getDidAddress } = await import("./exports-IETPJSYL.js");
          const chains = (_l = this.caipNetworks) == null ? void 0 : _l.filter((network) => network.chainNamespace === "eip155").map((chain2) => chain2.caipNetworkId);
          siweParams.chains = (_m = this.caipNetworks) == null ? void 0 : _m.filter((network) => network.chainNamespace === "eip155").map((chain2) => chain2.id);
          ...
          const signedCacao = (_n = result == null ? void 0 : result.auths) == null ? void 0 : _n[0];
          if (signedCacao) {
        ...
        const chainId = (_o = this.appKit) == null ? void 0 : _o.getCaipNetworkId();
      ...
      disconnect: async () => {
        ...
        if ((_c2 = (_b2 = (_a2 = this.options) == null ? void 0 : _a2.siweConfig) == null ? void 0 : _b2.options) == null ? void 0 : _c2.signOutOnDisconnect) {
          const { SIWEController } = await import("./exports-IETPJSYL.js");
  ...
  async syncAccount({ address, chainId, connector, addresses, status }) {
    ...
    if (status === "disconnected") {
      ...
      (_e = this.appKit) == null ? void 0 : _e.setStatus(status, this.chainNamespace);
      (_f = this.appKit) == null ? void 0 : _f.setLoading(false);
      SafeLocalStorage.removeItem(SafeLocalStorageKeys.WALLET_ID);
      if (isAuthConnector) {
    ...
    if (this.wagmiConfig) {
      if (connector) {
        if (connector.name === "WalletConnect" && connector.getProvider && address) {
          const activeCaipNetwork = (_g = this.appKit) == null ? void 0 : _g.getCaipNetwork();
          const currentChainId = chainId || (activeCaipNetwork == null ? void 0 : activeCaipNetwork.id);
          const provider = await connector.getProvider();
          const namespaces = ((_h = provider == null ? void 0 : provider.session) == null ? void 0 : _h.namespaces) || {};
          const namespaceKeys = namespaces ? Object.keys(namespaces) : [];
          const preferredAccountType = (_i = this.appKit) == null ? void 0 : _i.getPreferredAccountType();
          namespaceKeys.forEach((key) => {
            ...
            (_d2 = this.appKit) == null ? void 0 : _d2.setStatus(status, chainNamespace);
          });
          if (((_k = (_j = this.appKit) == null ? void 0 : _j.getCaipNetwork()) == null ? void 0 : _k.chainNamespace) !== ConstantsUtil.CHAIN.SOLANA && currentChainId) {
          ...
          (_m = this.appKit) == null ? void 0 : _m.setLoading(false);
          ...
          await Promise.all([
            this.syncConnectedWalletInfo(connector),
            (_n = this.appKit) == null ? void 0 : _n.setApprovedCaipNetworksData(this.chainNamespace)
          ]);
          (_o = this.appKit) == null ? void 0 : _o.setLoading(false);
          (_p = this.appKit) == null ? void 0 : _p.setCaipAddress(caipAddress, this.chainNamespace);
          (_q = this.appKit) == null ? void 0 : _q.setStatus("connected", this.chainNamespace);
          if (!isAuthConnector && (addresses == null ? void 0 : addresses.length)) {
            (_r = this.appKit) == null ? void 0 : _r.setAllAccounts(addresses.map((addr) => ({ address: addr, type: "eoa" })), this.chainNamespace);
          }
        } else if (status === "reconnecting") {
          (_s = this.appKit) == null ? void 0 : _s.setLoading(true);
        }
      }
    }
  }
  ...
  syncConnectors(_connectors) {
    ...
    filteredConnectors.forEach(({ id, name, type: type6, icon }) => {
      ...
      if (!shouldSkip) {
        const injectedConnector = id === ConstantsUtil3.INJECTED_CONNECTOR_ID;
        w3mConnectors.push({
          id,
    ...
    if (bypassWindowCheck || typeof window !== "undefined" && connector) {
      ...
      provider.onRpcRequest((request) => {
        ...
        if (W3mFrameHelpers.checkIfRequestExists(request)) {
          ...
          (_b = this.appKit) == null ? void 0 : _b.open();
      ...
      provider.onRpcError(() => {
        var _a, _b, _c, _d;
        const isModalOpen = (_a = this.appKit) == null ? void 0 : _a.isOpen();
        if (isModalOpen) {
      ...
      provider.onNotConnected(() => {
        var _a, _b, _c;
        const isConnected = (_a = this.appKit) == null ? void 0 : _a.getIsConnectedState();
        ...
        if (!isConnected && isConnectedWithAuth) {
          (_b = this.appKit) == null ? void 0 : _b.setCaipAddress(void 0, this.chainNamespace);
      ...
      provider.onConnect((user) => {
        ...
        (_b = this.appKit) == null ? void 0 : _b.setSmartAccountDeployed(Boolean(user.smartAccountDeployed), this.chainNamespace);
      ...
      provider.onGetSmartAccountEnabledNetworks((networks) => {
        var _a;
        (_a = this.appKit) == null ? void 0 : _a.setSmartAccountEnabledNetworks(networks, this.chainNamespace);
      });
    }
  }
